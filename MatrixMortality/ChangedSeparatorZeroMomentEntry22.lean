...
/-!
# Rank-nine zero-moment certificate, entry 22
...
/-- Entry (2, 2) of the chain chart's zeroth moment. -/
theorem chain_zero_moment_entry_two_two (ρ V K : ℚ)
    (regular : RegularChart ρ V K) :
    (chainOutput ρ V K * chainInput ρ V K) 2 2 = 1 := by
  ...
  have reversedWidthFactor_ne_zero : ρ * 3 - 1 ≠ 0 := by
    simpa [mul_comm] using regular.widthFactor_ne_zero
  ...
  simp
  simp only [RegularChart.subNegTailGap_eq, mul_neg, div_neg]
