...
| [`G3-M01`](#g3-m01-free-group-discrepancy-engine) | partial mechanism | Carvalho's fixed transducer reduces halting to one positive marker-tail equation with an all-path converse | audited | active |
...
For the exact fixed cyclic-tag program, Carvalho's inverse transducer carries the queue in the
freely reduced discrepancy
...
appends the production. An `H` marker counts simulated steps and a unique `p` marker prevents
trivial discrepancy on false paths. The fixed subgroup is trivial on nonhalting instances and
infinite cyclic on halting instances.

Rotating the terminal marker word puts the complete positive source in one equation:

```text
e halts ↔ ∃τ∈{0,1,H,p}*, n≥0: τ p H^n=d_e Ψ(τ).
```

First-letter cancellation proves the converse for every positive `τ`; it does not assume that
the input was pre-parsed into legal transition blocks.

**Scope:** one fixed four-letter sequential transducer, a source-dependent positive prefix
`d_e`, and a unary marker tail. The high-rank Stallings presentation and its positive basis are
dead by [`M4-O17`](#m4-o17-positive-nielsen-basis-obstruction). No positive binary
fixed-boundary compiler follows yet.

**Use:** this marker-tail equation is the source for the boundary punctuation mechanism
[`M4-M05`](#m4-m05-boundary-guarded-homogeneous-punctuation). Preserve its all-word converse;
do not return to basis compression.

**Source:** [`carvalho-2026-free-group-pcp.md`](references/carvalho-2026-free-group-pcp.md) and
[`audits/m43-free-group-punctuation-2026-08-11.md`](audits/m43-free-group-punctuation-2026-08-11.md).

**Next:** compile the equation to positive binary fixed-boundary free-group equality, or prove
that restricted problem decidable and close the route.
