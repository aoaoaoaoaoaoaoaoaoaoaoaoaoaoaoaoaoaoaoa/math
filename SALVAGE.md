...
| [`M4-O08`](#m4-o08-closed-phase-two-ray-wall) | obstruction | exceptional and residue-zero atoms preserve a nonvanishing two-ray quotient | formalized | graduated |
...
**Scope:** this constructs and classifies the matrix mechanism. The closed-phase wall
[`M4-O08`](#m4-o08-closed-phase-two-ray-wall) now proves that every word confined to `R` and
`Q(x,3j)` is nonzero. It does not supply open contexts whose boundary rays realize the paired
Neary zero series, nor classify bridge zeros containing a regular gap of residue one or two. No
`M₄(3)` theorem follows yet.

**Use:** all further cube-root work should begin with the first regular open-residue block in the
bridge language. Do not redo finite-gap searches, rank-one incidence, or closed-phase products.

**Next:** lift the exceptional modulo-three states created by residue-one and residue-two atoms.
...
### M4-O08: Closed-phase two-ray wall
...
Clear the uniform denominator eight from every closed atom `Q(x,3j)` and from the exceptional
atom `R=Q(b,1)`, then reduce modulo three. Every closed atom becomes
...
Both preserve the two rays generated by `e₀,e₂` with nonzero weights: `C` resets either ray to
`e₀`, and `E` resets either to `e₂`. Every word in `{C,E}` therefore acts nontrivially on `e₀`.
Scalar extension then proves that every rational atom word over `R` and `Q(x,3j)` is nonzero.

**Scope:** every natural `β`, every body, every `j≥0`, arbitrary length, arbitrary exceptional
count, adjacent exceptional atoms, and the empty word. The theorem does not cover a regular atom
`Q(x,3j+1)` other than `R`, or any `Q(x,3j+2)`.

**Use:** a parabolic zero must cross the open phase through a regular residue-one or residue-two
atom. Closed contexts cannot realize even a same-zero semantic boundary. The next attack begins
at the first such atom and the exceptional residue state it creates.

**Next:** classify exact lifts of the residue states `0` and `𝔽₃(1,-1)` in the bridge automaton;
construct the intended boundary or prove an unavoidable malformed lift.

**Artifact:** `ParabolicBlade.closedPhase_wordProduct_ne_zero` in
[`MatrixMortality/ParabolicClosedPhase.lean`](MatrixMortality/ParabolicClosedPhase.lean) and
[`audits/m43-closed-phase-2026-08-05.md`](audits/m43-closed-phase-2026-08-05.md).
