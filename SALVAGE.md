...
### MM-O16: Exact delimiter-pair obstruction
...
### MM-O28: Tilted geometric-tail rank-nine wall
...
It does not classify the residue-two diagonal; `MM-S41` and `MM-S43` expose its final four-`c`
macro, and `MM-S48` later proves that every residue-two diagonal source halts. Bodies with unequal
...
the direct target-ratio chamber of `MM-S51`. Its normalized discrepancy is generally rational;
`MM-S52` separately proves that no unconditional local carrier-to-fringe adapter can make it
integral.
...
directly from its rational cylinder and the target block's full upper/lower ratio. Do not apply
the integral raw-fringe classifier without an orbit-specific ancestry theorem.
...
`MM-S34`, it reduces the diagonal to `n≡2 (mod 9)`; `MM-S41` and `MM-S43` expose the last
four-`c` macro, which `MM-S48` proves always halts. Unequal separated
...
**Next:** `MM-S48` closes the residue-two diagonal. Continue with unequal separated runs.
...
`MM-S30`, it reduces the diagonal to `n≡2 (mod 9)`; `MM-S41` and `MM-S43` expose the last
four-`c` macro, which `MM-S48` proves always halts. The result does not
...
**Next:** `MM-S48` supplies the finite injective quotient and closes the residue-two diagonal.
Continue with unequal separated runs.
...
**Scope:** this refines only the residue-two diagonal `bb c bⁿ c bⁿ`. `MM-S43` exposes its
recursive block and `MM-S48` subsequently closes the entire diagonal. Unequal separated
...
**Next:** `MM-S43` derives the post-six-event macro; `MM-S48` closes its centered block dynamics.
...
does not itself decide `n≡38,65 (mod 81)`, unequal separated runs, arbitrary two-`c` bodies, or the
...
**Next:** `MM-S48` derives the exact one-block and two-block return maps and proves extinction.

### MM-S48: Centered four-c extinction

**Kind:** structure theorem and halting theorem
**Evidence:** formalized
**Disposition:** active

Write the last `MM-S41` class as `n=27a+11`, set `A=9a+4`, and let `B_A` be the `MM-S43`
four-`c` block. The six-event entry reaches

```text
B_A b^(82a+38).
```

Every later active queue normalizes to either one block `O(T)=B_A b^T` or two blocks
`P(T)=B_A b^(39a+19) B_A b^T`. Put

```text
s = 105a+49,    q = 129a+62,    x = T-s.
```

The exact four-`c` context macro and head-clean prefix deletion give the partial dynamics
...
The open interval `−q<2x<q` is invariant and finite over the integers. Within it, each live
state has at most one predecessor: idle-branch images lie in the central third, while crossing
images lie in the appropriate outer third. The initial state is `O(−23a−11)` and has no
predecessor in the interval. Deleting a predecessor-free root from a finite injective partial
graph recursively proves accessibility, so every orbit reaches a missing residue and the tag
queue drains.

Consequently every `n=27a+11` coupled source halts. Combining this with `MM-S30`, `MM-S34`,
and `MM-S41` proves that every diagonal source with `n≡2 (mod 3)` halts; `MM-S25` supplies
exact periodic nonhalting orbits in the other two residues for `n>0`. A direct two-step entry
puts the degenerate `n=0` source on its own exact two-state cycle. Thus halting is equivalent to
`n≡2 (mod 3)` for every natural `n`.

**Scope:** this completes the coupled diagonal family `bb c bⁿ c bⁿ`. It does not classify
unequal outer runs, arbitrary separated two-`c` bodies, or the full width-three source problem.

**Use:** future separated-two-`c` work should quotient block populations before refining
congruences. The centered interval and predecessor-free-root lemma apply whenever the context
macro closes on one and two blocks with a fixed translation span.

**Formalization:**
[`MatrixMortality/SeparatedTwoCDiagonal.lean`](MatrixMortality/SeparatedTwoCDiagonal.lean),
through `SeparatedTwoCDiagonal.fourCCentered_initial_accessible`,
`SeparatedTwoCDiagonal.elevenModuloTwentySeven_tagHaltsFrom`, and
`SeparatedTwoCDiagonal.diagonal_tagHaltsFrom_iff`. The arbitrary-suffix block macro is
`SeparatedTwoCResidue.fourCBlock_bRun_reaches_final`.
...
**Next:** the two later-frontier collisions graduate into
[`MM-S65`](#mm-s65-complete-all-c-rule-entry-extinction).

**Issue:** [#6, Formalize the five-state setter candidate and decide projective
avoidance](https://github.com/aoaoaoaoaoaoaoaoaoaoaoaoaoaoaoaoaoaoaoa/math/issues/6).

### MM-S65: Complete all-c rule-entry extinction

**Kind:** obstruction
**Evidence:** formalized
**Disposition:** active

After `MM-S62`, the all-`c` rightmost-rule grammar has two later frontiers. At a regular raw
head with final-seven width `h`, the erasure-tail width must equal `h`. Scaling by `81` exposes
normalized residue `2^h` on both the all-erasure companion and the phase perturbation. Their
sum has residue `2·2^h`, a five-adic unit, so the rule residual has exact five-depth `h+1`.
At least two roles precede the rule, making the physical target at least `h+2`.

At the exceptional head, the erasure-tail width is `2β−2`. Scaling by `45` puts both arms at
depth `2β`. The all-erasure coefficient is `2` or `4`, while the phase coefficient is `2`;
their sum is `4` or `6` modulo five. The scaled rule residual therefore has exact depth `2β`,
whereas the scaled physical target has depth at least `2β+1`.

`allCRightmostRule_rawHead_shell_impossible` first invokes the `MM-S62` grammar adapter. It
dispatches leading and position-two rules through `MM-S54` and `MM-S62`, proves that every
remaining rule has at least two predecessors, and applies the appropriate coefficient
collision. Hence every all-`c` rightmost-rule distinguished raw-head entry is impossible.

**Scope:** the theorem covers every all-`c` block with a rightmost rule followed by erasures,
from the distinguished decimal-unit two-`c` raw head to the physical multi-role shell. It does
not cover b-bearing rules, generalized later carriers, or singleton targets.

**Use:** combine with [`MM-S60`](#mm-s60-complete-b-bearing-rule-entry-extinction) to remove the
complete rightmost-rule branch from the distinguished first-entry grammar.

**Formalization:**
[`MatrixMortality/DecimalSetterAllCRule.lean`](MatrixMortality/DecimalSetterAllCRule.lean),
through `laterAllCRightmostRule_rawHead_shell_impossible` and
`allCRightmostRule_rawHead_shell_impossible`.
...
**Next:** `G3-S14` completes the five-rule quotient. Classify its parametric fork equation, then
test any non-common-fixed solution against the unchanged endpoint and mantissa converse.

### G3-S14: Finite convergent mixed-prime presentation

**Kind:** structure theorem, decision procedure, and separation

**Evidence:** exact independently replayable certificate; Lean fork witness

**Disposition:** active

Orient the five audited mixed-prime relations by equal-length lexicographic descent with `D<T`.
Their `45` proper critical branch pairs are distinct; adjoining those pairs in the same orientation
produces `50` rules. Every rule preserves length and strictly decreases its word, so reduction
terminates. The completed left sides are distinct and contain no proper inclusion. Exact
enumeration finds `450` proper overlaps, at source lengths `52` through `88`. All join: the `45`
adjoined pairs require one reduction on one branch, while each of the `405` new pairs requires two
reductions on one branch and none on the other. The critical-pair lemma gives local confluence;
Newman's lemma gives confluence.

The `45` added equations are branch pairs of base critical peaks, so they do not enlarge the
original congruence. The completed system is therefore a finite convergent presentation of the
congruence generated by the five relations. Every fixed word has one irreducible normal form, and
two fixed words are congruent exactly when those forms agree. The canonical certificate contains
every rule, critical source, branch, trace, and common normal form. Its SHA-256 is

```text
f3f424457fe1b0ad9b82e92f6b2168b46b20e7a975b57a6a7fc497afde4520b3.
```

The system settles quotient-fork existence affirmatively. The `G3-S13` length-`312` pair
normalizes to one word in four steps on the flat side and five on the nested side; the common
normal form has SHA-256
`71e801595934c689b15a86d0f20096f22a0a993144514b791f3b167ec068fb9c`.
Lean independently proves the pair's critical factorization and its endpoint-fatal `F,F²,F⁴`
action collapse.

The presentation is not the full affine kernel. Every completed left side begins `TTDD` and has
length at most `59`. For every positive depth, both sides of the odd kernel family are distinct
irreducible words: an occurrence wholly inside the alternating `(DT)^k` pump cannot begin
`TTDD`, while the finitely many boundary factors stabilize by depth `59` and are exhaustively
checked. Unique normal forms therefore separate the two sides for every `k≥1`.

**Scope:** confluence decides congruence of every supplied fixed pair. It does not by itself decide
the existential parametric equation `YZXYX≡XZYXY`, nor prove that every quotient fork is
common-fixed. The quotient contains the known common-fixed fork; whether it contains a usable
non-common-fixed one remains open. The positive-depth odd family and six other audited
length-`31` affine classes lie outside the five-rule presentation.

**Use:** normalize every proposed five-rule quotient witness instead of extending a
nonconfluent first-generation search. Reject the positive-depth odd family as a source of hidden
multi-step derivations in this quotient. Keep full affine-kernel relations and quotient relations
separate.

**Artifact:**
[`certify_mixed_prime_completion.py`](tools/certify_mixed_prime_completion.py) and
[`m34-mixed-prime-finite-completion-2026-08-31.md`](audits/m34-mixed-prime-finite-completion-2026-08-31.md),
composed with
[`GuardedMixedPrimeForkQuotient.lean`](MatrixMortality/GuardedMixedPrimeForkQuotient.lean).
...
### D2-O08: Real-trap backward saturation
