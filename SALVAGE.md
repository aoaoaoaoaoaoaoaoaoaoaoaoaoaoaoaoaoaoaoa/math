...
| [`D2-O04`](#d2-o04-forced-exit-surface) | obstruction and decidable exit surface | every next block leaves the accepted `43/24` collision ray at valuation minus one; its two-parameter exit surface is injective and has a two-candidate fixed-target test | formalized | active |
...
**Next:** the target-pole intersection and the ray's own fixed-target query are closed. The first
forced-exit extension is [`D2-O04`](#d2-o04-forced-exit-surface); attack its post-exit fixed-length
suffix relation and whether the prescribed benchmark target lies there. Then seek other adjacent
collision rays not conjugate to this one.

### D2-O04: Forced-exit surface

**Kind:** obstruction and decidable exit surface
**Evidence:** formalized
**Disposition:** active
...
**Scope:** this decides fixed-target membership only in the one-block exit surface generated by
the `D2-O03` collision ray. It does not represent exits from arbitrary reachable shell states.
For a further tail, the target valuation fixes its length, but intersection of this parametric
surface with the resulting fixed-count reachability relation remains open. No mortality or full
`M₂(3)` decision theorem follows.
...
fibre by one uncontrolled exit block. The first exit is forced and exactly parameterized; the
remaining infinite seam starts with arbitrary pre-exit states or a nonempty post-exit suffix.

**Next:** fix the suffix length from the target's five-adic valuation and eliminate the remaining
wait variables, or exhibit a post-exit collision family that survives a specified target. In
parallel, test whether the benchmark target lies on any finite controlled extension of this
surface.
