...
residue ring. Hence neither one modulus nor any finite family supplies a residue-orbit
...
singular spelling memory, and `R32-O22` removes congruence separation.
...
its orbit modulo every positive integer. Thus the shared artery must use rational/global word
...
6. Formalize `D2-S02` and `D2-D05`–`D2-D07`, use `D2-D10` to remove real exterior targets, then
   ...
   infinite seam, while `D2-O06` saturates target-only reverse length inside the real trap. Treat
   chamber exit and reentry as separate pre-exit seams. Do not reopen unlabeled residue-only
   finite nuclei, uniformly bounded reverse fanout, one-sided endpoint bounds, or global real drift.
