...
2. **Transverse line-atlas escape.** Build a source-computable arbitrary-body recognizer using
   ...
   of those dead endpoints. The unresolved step is a relation with a proper kernel subpair,
   genuinely multi-cut or stateful address/cloak production and complete endpoint converse,
   separate two-offset routing, or a genuinely multi-window
...
different proposed cuts on the two relation sides. A new relation with a proper kernel subpair,
multi-cut/stateful realization, or separate two-offset route, plus the all-word endpoint converse,
remains.
