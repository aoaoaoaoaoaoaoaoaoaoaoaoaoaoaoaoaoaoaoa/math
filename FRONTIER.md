...
2. **Transverse line-atlas escape.** Build a source-computable arbitrary-body recognizer using
   ...
   alternative: any genuine kernel pair cloaks a raw-distinct arbitrary address comparator. The
   unresolved step is its physical three-macro factorization and complete endpoint converse,
   separate two-offset routing, or a genuinely multi-window non-common-fixed quotient fork, not
...
asymmetric reader now exists abstractly by `G3-S21`; its physical three-macro factorization, or a
separate two-offset route, and the all-word endpoint converse remain. After the missing guard and
closure proofs,
