...
dimension. The source seam now has one exact internal boundary: for every `n>0` with
...
2. Fuse the paired toggle and separator inside one five-dimensional generator. The
   ...
   [`MM-O15`](SALVAGE.md#mm-o15-exact-delimiter-pair-obstruction) proves that exact parity-pair
...
3. Use the scheduled compiler [`MM-C03`](SALVAGE.md#mm-c03-scheduled-binary-compiler).
   ...
   machine with an arbitrary input. [`MM-S25`](SALVAGE.md#mm-s25-separated-two-c-periodic-orbits)
