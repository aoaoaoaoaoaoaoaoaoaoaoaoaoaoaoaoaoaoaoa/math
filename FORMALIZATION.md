...
| [`D2-O06`](SALVAGE.md#d2-o06-real-trap-backward-saturation) | exact rational predecessor cover of the half-open real trap, preservation of `5`-adic units in reverse, and guarded predecessor schedules of every prescribed block length |
| [`D2-O07`](SALVAGE.md#d2-o07-guarded-real-pole-reset) | exact normalized band coordinates, full deepest-branch depth reset, an unbounded period-fifty reset family preserving the real and `5`-adic guards, its fixed-source feeder, two-/three-adic profile, and target injectivity |
