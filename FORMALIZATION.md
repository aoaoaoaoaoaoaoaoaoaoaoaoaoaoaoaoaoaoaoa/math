...
| [`D2-O09`](SALVAGE.md#d2-o09-guarded-real-pole-reset) | exact normalized band coordinates, full deepest-branch depth reset, and an unbounded period-fifty reset family preserving the real and `5`-adic guards |
...
| `MixedPrimeRealTrapReset.lean` | full normalized pole branch and its unbounded period-fifty guarded depth-reset family |
...
| Every sufficiently deep normalized source band occurs on the deepest real-trap predecessor branch; a period-fifty subfamily preserves both endpoint unit guards at unbounded depth | `MixedPrimeDebt.realTrapMaxPredecessorWait_bandPoint`, `MixedPrimeDebt.shellStep_realTrap_poleBranch_full`, `MixedPrimeDebt.shellStep_realTrap_guardedPoleReset` |
