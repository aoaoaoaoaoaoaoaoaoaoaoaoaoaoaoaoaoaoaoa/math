...
The dimension-two affine ledger has sixteen independently checked records:
...
| [`D2-S07`](SALVAGE.md#d2-s07-period-ten-shell-guard) | exact period-ten acceptance, the tail-dependent period `2·5^(ℓ+1)`, finite residue reduction, and sharpness by unit zero preimages |
...
| At every `5`-adic unit source, one-step shell acceptance depends only on the wait modulo ten; a fixed length-`ℓ` tail has period `2·5^(ℓ+1)`, and lowering the precision exponent by one fails at an explicit unit zero preimage | `MixedPrimeDebt.shellStep_fiveUnit_add_ten_iff`, `MixedPrimeDebt.shellStep_fiveUnit_add_ten_mul_iff`, `MixedPrimeDebt.shellStep_fiveUnit_iff_mod_ten`, `MixedPrimeDebt.shellRun_tail_fiveUnit_add_precisionPeriod_iff`, `MixedPrimeDebt.shellRun_tail_fiveUnit_add_precisionPeriod_mul_iff`, `MixedPrimeDebt.shellRun_tail_fiveUnit_iff_mod_precisionPeriod`, `MixedPrimeDebt.shellRun_shellZeroPreimage`, `MixedPrimeDebt.shellZeroPreimage_fiveUnit`, `MixedPrimeDebt.shellRun_tail_precisionPeriod_sharp` |
...
class: `n≡2,20 (mod 27)`. Only `n≡11 (mod 27)` remains open on the diagonal. No declaration
claims that the full two-`c` stratum is universal or decidable.
