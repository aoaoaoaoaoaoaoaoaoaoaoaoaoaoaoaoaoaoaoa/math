...
Rewrite-system completion is now exact audited mathematics. With `D<T`, every checked rule is
oriented from its lexicographically larger side to its smaller side; equal length makes
termination immediate. The five-rule basis has `45` nonjoinable proper critical overlaps and is
not confluent by itself. Adjoining those `45` branch pairs yields a `50`-rule system with no
inclusion ambiguities and `450` proper overlaps, all joinable. The critical-pair lemma and Newman's
lemma give a finite convergent presentation of the congruence generated by the five rules. The
independent certificate stores every critical trace and canonical normal form in
[`certify_mixed_prime_completion.py`](tools/certify_mixed_prime_completion.py).

Exact enumeration of all raw words proves that the five-rule congruence accounts for every affine
collision through length 30; seven independent collisions appear among the `2^31` words of length
31. One is the formal index-one odd-family member; six remain computational. Unique completed
normal forms prove that every positive-depth odd-family relation lies outside this congruence.
The exact enumerator and small-radius cross-check remain in
[`audit_mixed_prime_kernel.rs`](tools/audit_mixed_prime_kernel.rs); its self-check and the finite
completion certificate are part of the canonical gate, while the multi-billion-word census is
not. The presentation is complete for the generated congruence, not the affine kernel.
Fixed-source point collisions add a separate stabilizer problem. See
...
The arbitrary-composite residue is now primewise rigid on its numerator side. Every nondegenerate
...
The first center re-entry parity is also checked. A zero equal-scale residue maps to
`A−t²(A−B)`. Requiring the next scale `r` to be an exact denominator center gives
...
[`D2-O05`](SALVAGE.md#d2-o05-promised-empty-free-orbit-inverse-cycle) marks the exact boundary of
that height ratchet. Lean proves strict ping-pong for `D(z)=5z` and the transverse parabolic
`U(z)=(-2z+3)/(-3z+4)`, trivial stabilizer of the source `[9:4]`, and faithfulness of the
two-cyclic-factor free-product representation. The nonidentity positive word `UD` fixes `[3:5]`;
conjugating it by a hypothetical source-to-target word proves nonreachability and emptiness of the
target stabilizer coset. Chamber-directed inverse stripping follows `[3:5]→3→[3:5]`, while the
coprime primitive heights follow `5→3→5`. Lean also checks the integral conjugator carrying
...
[`D2-O10`](SALVAGE.md#d2-o10-finite-bounded-prefix-horizon) makes the bounded branch finite.
The integer square containing every primitive pair of height at most `H` has exactly
`(2H+1)²` elements. Hence `(2H+1)²+1` distinct prefixes bounded by `H` already expose a
nonidentity target stabilizer, while a trivial target stabilizer forces height above `H` within
that window. The bound deliberately counts nonprimitive, zero, and opposite-sign pairs; it is a
uniform containing bound, not a sharp ray count.

[`D2-O11`](SALVAGE.md#d2-o11-bounded-branch-nonreachability-certificate) composes that collision
with the unique-coset source invariant. A nonidentity target stabilizer conjugates through any
hypothetical source-to-target transporter to a nonidentity source stabilizer. Hence, under trivial
source stabilizer, a bounded window directly certifies nonreachability. Every window of the
explicit length therefore either escapes above `H` or proves that no transporter exists.

[`D2-O12`](SALVAGE.md#d2-o12-exact-prefix-height-rate) gives the exact finite-family form of
the escape rate. At a trivial-stabilizer target, `N` distinct prefixes represented at primitive
height at most `H` satisfy `N≤(2H+1)²`. Equivalently, `(2H+1)²<N` forces at least one state above
...
Record [`MM-O27`](SALVAGE.md#mm-o27-reachable-predecessor-cylinder) proves that no block-local
extinction is possible. At `β=3`, body `bbcc`, the ordinary-reset history
`[R_c,D_c]²` reaches an explicit primitive carrier. A third `[R_c,D_c]` normalizes by `−3`,
...
prefix discrepancy equals `λq₂`. The final unequal digit makes that discrepancy a unit without
any pole hypothesis; the braid forces the initial residual to be a unit. Lean then pulls the
same digit through both deletion denominators and the initial physical denominator.
...
| [`D2-S13`](SALVAGE.md#d2-s13-guarded-upper-run-cycles-of-every-period) | the full `L_n,U₀^r,U₁,M_m` family, exact interval and branch identities, complete prefix guards, two-adic phase strata, phase separation, and primitive guarded cycles of every period at least three |
...
| `MixedPrimeMacroAddress.lean` | free binary affine submonoid generated by `DT,TD`, with exact offset code and two-sided modular decoding |
...
| In a group, a contextual equality is exactly commutation with one connector; in dimension two, every matrix power lies in `span{I,S}`, so two side-contextual word-pump seeds propagate even without invertibility | `TwoSeedPumpPropagation.group_context_eq_iff_commute`, `TwoSeedPumpPropagation.group_context_pow_eq_of_zero_one`, `TwoSeedPumpPropagation.finTwo_cayleyHamilton`, `TwoSeedPumpPropagation.finTwo_pow_mem_span_one_self`, `TwoSeedPumpPropagation.finTwo_linearContext_pow_eq_of_zero_one`, `TwoSeedPumpPropagation.wordProduct_pump_eq_of_zero_one` |
| The macros `DT,TD` have an exact mixed `2/5` radix code decoded from the left modulo two and from the right modulo five | `MixedPrimeMacroAddress.addressOffset_scaled`, `MixedPrimeMacroAddress.addressCode_cons_mod_two`, `MixedPrimeMacroAddress.addressCode_append_cast_zmod_five`, `MixedPrimeMacroAddress.addressCode_injective_of_length_eq` |
| Equality of affine actions recovers the whole binary macro address, including its length; one constant-address offset selects exactly that address | `MixedPrimeMacroAddress.expandAddress_wordAction_injective`, `MixedPrimeMacroAddress.eq_replicate_of_addressOffset_eq` |
| The seven length-31 pump families have an exact all-depth balanced-prefix census; aligned next and previous letters isolate only family-six cuts 27 and 28 | `GuardedMixedPrimeFork.PumpedKernelFamily.balancedPrefix_iff_admissible`, `GuardedMixedPrimeFork.PumpedKernelFamily.alignedNext_of_balancedPrefix`, `GuardedMixedPrimeFork.PumpedKernelFamily.alignedPrevious_of_balancedPrefix`, `GuardedMixedPrimeFork.PumpedKernelFamily.no_twoPreviousAligned_balancedPrefix` |
| Every moving internal/internal literal fork cell is impossible, while the family-six cut-27 residue is bounded by power 11 and the cut-28 right-conjugacy repair fails uniformly | `GuardedMixedPrimeFork.no_dynamicInternal_literalPumpedContextFork`, `GuardedMixedPrimeFork.SixthFamilyCutTwentySevenResidue.power_le_eleven`, `GuardedMixedPrimeFork.sixthFamily_cutTwentyEight_no_rightConjugacy`, `GuardedMixedPrimeFork.sixthFamily_weakCutTwentySeven_block_ne` |
| Evaluation at zero alone recovers an arbitrary finite `DT/TD` address: zero isolates empty, and the reduced five-adic denominator recovers every positive length | `MixedPrimeMacroComparator.five_not_dvd_addressCode`, `MixedPrimeMacroComparator.addressOffset_injective`, `MixedPrimeMacroComparator.expandAddress_at_zero_injective` |
...
| Every diagonal separated body with `n≡2,20 (mod 27)` has a halting coupled input | `SeparatedTwoCResidue.twoModuloTwentySeven_tagHaltsFrom`, `SeparatedTwoCResidue.twentyModuloTwentySeven_tagHaltsFrom` |
| A canonical four-active-`c` block has an exact two-copy expansion | `SeparatedTwoCResidue.fourCQueue_reaches_final` |
| Every diagonal separated body with `n≡11 (mod 81)` has a halting coupled input | `SeparatedTwoCResidue.elevenModuloEightyOne_tagHaltsFrom` |
| A canonical block reproduces across any suffix beginning with at least two unary letters | `SeparatedTwoCResidue.fourCBlock_bRun_reaches_final` |
| The centered one/two-block macro is accessible from its initial defect | `SeparatedTwoCDiagonal.fourCCentered_initial_accessible` |
...
certified rejected collision suffix reaches the forced target. At the `G3-S08` boundary, the
remaining branch was a non-common-fixed macro triple with a literal morphic relation or a genuine
kernel relation; `G3-S10` closes the literal alternative.

`G3-S10` closes the literal alternative. Cancelling the common macro context reduces literal
flat/nested equality to `yzxyx=xzyxy`. Equal-length splitting gives `yzx=xzy` and `yx=xy`.
Since exactness makes the `x` and `y` affine actions distinct, one word is nonempty; its explicit
rational fixed point is also fixed by the other action. Evaluating `yzx=xzy` there shows that
`x` and `y` agree at its image under `z`. Unless `z` fixes the point, two-point affine rigidity
makes the `x` and `y` actions identical. Thus all three macros share one fixed point, contradicting
`G3-S08`. Lean consequently strengthens the fork dichotomy to a mandatory pair of distinct raw
...
`G3-S14` completes the five-rule quotient. Orienting those `45` critical branch pairs produces a
terminating `50`-rule system with no inclusion ambiguities. An independently replayable exact
certificate enumerates and joins all `450` proper overlaps: `45` close in one step and `405` in
two. Thus the completed rules are a finite convergent presentation of the original congruence,
and normal-form equality decides every fixed congruence query. The certificate replays the
length-`312` fork in four versus five reductions. It also proves that both sides of every
...
`G3-S16` supplies a new external-kernel mechanism. An exhaustive `2³²`-word census has `44`
independent duplicate edges; cancelling maximal literal contexts gives seven length-`31` and
sixteen length-`32` cores. Each core has one side-contextual `DT` or `TD` insertion whose depth
zero and one signatures agree. Lean's generic `2×2` Cayley-Hamilton theorem propagates every
schema to all depths. A finite periodic-window certificate proves that all `23` schemas remain
irreducible under the completed `50` rules for every depth.

For invertible contexts Lean gives the exact group explanation. The depth-zero equality defines
one connector `P=A⁻¹B`, and `AWC=BWD` is equivalent to `WP=PW`. Thus every raw pump schema is a
centralizer reader; Cayley-Hamilton extends its two-seed propagation to arbitrary commutative-ring
matrices without invertibility.

The same formalization proves that the length-two macros `DT,TD` generate a free binary affine
submonoid. Their common multiplier recovers address length. An exact integer offset code is
decoded from the left modulo two and from the right modulo five, recovering every digit. Each of
the `23` contextual schemas nevertheless accepts only the constant address formed from its chosen
pump: exact alternate-pump substitution is nonzero in every case. A general binary address reader,
not storage, is now the missing interface.

`G3-S17` classifies the balanced cuts in the seven length-`31` pump schemas uniformly at every
depth. Families three, five, and seven have only cut `3`; families one, two, and four add the
moving cuts `30+2i`; family six adds those moving cuts and the static cuts `27,28`. Lean proves
that adjacent-letter agreement at a balanced cut isolates cut `27` on the right or cut `28` on
the left, and that two preceding letters never align.

The moving internal/internal cell is now formally empty at every depth. Length balance forces the
last moving cut and one of the three layouts `(1,2,0,0)`, `(2,1,0,0)`, or `(2,2,1,1)` for
`(|X|,|Y|,|P|,|Q|)`; the literal fork equations then demand incompatible slices of the fixed
prefixes `DTTT` and `TTDD`. Families three, five, and seven are already arithmetically empty in
that cell. For family six, Lean bounds the remaining cut-`27` internal residue by pump power
`11`, and an exact union-find certificate rejects all `156` lawful geometries. Lean also rejects
the cut-`28` one-letter right conjugacy and transports a fixed `25`-letter mismatch across the
weak cut-`27` family. These are local semantic gates, not yet one global theorem extracting every
comparable or same-shorter layout, and they do not cover multi-window quotient conversions.

`G3-S18` supplies the general equality test missing from the free address stack. Lean proves that
the scalar `wordAction(expandAddress(w),0)` is injective for addresses of arbitrary length. The
empty address is isolated by zero; every nonempty code is a unit modulo five, so equality after
clearing denominators forces equal lengths before the fixed-length radix decoder recovers every
digit.

At equal address length, Lean proves two exact scalar comparators. The commutator words `UV` and
`VU` agree at state zero exactly when `U=V`. More importantly, for every arbitrary raw toggle
word `Z`, the reduced physical fork words `VZUVU` and `UZVUV` agree at state zero exactly when
`U=V`. The difference factors as `(u-v)(c(r³-r²+r)-1)` with `0<r<1` and `c≤1`, making the second
factor strictly negative. Unequal lengths cannot be inferred by this five-factor test: for
`A=DT`, the triple `U=A,V=A²,Z=A³` makes both branches `A⁹`, on the already forbidden common-fixed
diagonal. The remaining M₃(4) seam is no longer equality testing. It is dynamic synchronized
address production, or direct routing of two globally injective scalar offsets, together with
the complete malformed-word converse.

`G3-S19` records the resulting carrier tax. Existing endpoint rigidity forces the flat and nested
fork images to be distinct raw words with one affine action. If both images lie in the aligned
address submonoid, `G3-S18` identifies their addresses from their common value at zero and hence
identifies the raw words, a contradiction. Since address expansion preserves concatenation, this
rejects every code that assigns all three fixed controls whole `{DT,TD}` addresses, at arbitrary
and possibly zero lengths. Even with an arbitrary unaligned toggle, equal-length aligned data
macros are impossible because exact endpoint semantics forces their slopes to differ. Interior
address factors remain open: the global fork must break two-letter address phase around them, or
their offsets must reach the terminal gate through a separate channel.

`G3-S20` excludes the first phase-breaking repair. If the two forced fork words are
`P·expandAddress(u)·Q` and `P·expandAddress(v)·Q` for common literal contexts `P,Q`, bijectivity
cancels both contexts from their forced action equality. Scalar address injectivity then gives
`u=v`, and the raw fork words become equal, contradicting endpoint exactness. The addresses may
...
`G3-S32` gives the exact-linear boundary of that factor method. For each base mixed-prime
relation separately and every `2≤r≤6`, the integer lattice generated by all contextual
`Φᵣ`-moves is the full coordinate-sum-zero lattice. The exact certificate enumerates the
complete `r−1` boundary contexts, deletes one augmentation coordinate, and obtains identity
Hermite normal form in all twenty-five cells; modular ranks over four prime fields are checked
independently. Hence every abelian-group-valued additive invariant at one certified factor width
is merely total factor count. This is an audited exact arithmetic result, not a Lean theorem;
nonlinear, mixed-width, stateful, boundary-aware, and radius-at-least-seven invariants remain.
...
defect map and proves that every `n≡5 (mod 9)` coupled input halts.
`SeparatedTwoCResidueTwo.lean` then proves halting in two of the three subresidues of the last
class, `n≡2,20 (mod 27)`, derives the surviving four-`c` reproduction macro, and proves the
further class `n≡11 (mod 81)` halts. `SeparatedTwoCDiagonal.lean` closes the two surviving
...
residue pairs are further cut by a single six-active-`c` theorem: the matched pairs
