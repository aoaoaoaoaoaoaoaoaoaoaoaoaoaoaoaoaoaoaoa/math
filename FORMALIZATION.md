...
| [`D2-S24`](SALVAGE.md#d2-s24-first-length-three-chamber-cut) | the exact source and target throughout the closed positive-positive-negative length-three chamber, terminal-gauge independence of the source, the uniform real-trap reduction to `k∈{1,2}`, target-guard implication of the source guard, the depth-two target carry, exact ten-periodic terminal acceptance, the finite outer parameter cut, and forced even first displacement |
