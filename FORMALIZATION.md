...
| [`D2-O09`](SALVAGE.md#d2-o09-guarded-real-pole-reset) | exact normalized band coordinates, full deepest-branch depth reset, an unbounded period-fifty reset family preserving the real and `5`-adic guards, its two-/three-adic profile, and target injectivity |
...
| `MixedPrimeRealTrapReset.lean` | full normalized pole branch, its unbounded period-fifty guarded depth-reset family, exact two-/three-adic profile, and target injectivity |
...
| Every sufficiently deep normalized source band occurs on the deepest real-trap predecessor branch; a period-fifty subfamily preserves both endpoint unit guards at unbounded depth, has exact two-/three-adic values, and has distinct targets | `MixedPrimeDebt.realTrapMaxPredecessorWait_bandPoint`, `MixedPrimeDebt.shellStep_realTrap_poleBranch_full`, `MixedPrimeDebt.shellStep_realTrap_guardedPoleReset`, `MixedPrimeDebt.shellStep_realTrap_guardedPoleReset_twoThreeValues`, `MixedPrimeDebt.shellStep_realTrap_guardedPoleReset_target_injective` |
