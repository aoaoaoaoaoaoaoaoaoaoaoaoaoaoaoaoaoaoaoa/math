...
| [Dong24](dong-2024-semigroup-algorithmic-problems-metabelian.md) | peer-reviewed STOC 2024; retained arXiv v1 full version | Identity, Group, and Inverse Problems are decidable for finitely generated metabelian groups; target membership is outside the theorem |
| [Dong25](dong-2025-submonoid-membership-lamplighter.md) | peer-reviewed ICALP 2025; CC-BY | Submonoid Membership is decidable for finitely presented Laurent modules over a prime field; characteristic zero and rational subsets remain outside the theorem |
| [Serre70](serre-1970-congruence-sl2.md) | peer-reviewed Annals 1970; metadata-only local record | Exact congruence-subgroup property for `SL₂(ℤ[1/p])`: every finite-index subgroup contains a principal congruence kernel |
| [CGS17](chorna-geller-shpilrain-2017-two-generator-subgroups.md) | peer-reviewed Journal of Algebra 2017; metadata-only local record | Peak reduction decides complete-matrix membership in the two step-`k` shear subgroup for integral `k≥2`; it does not decide projective point orbits |
| [HMST24](han-masuda-singh-thiel-2024-subgroup-membership.md) | peer-reviewed Journal of Algebra 2024; metadata-only local record | Continued-fraction divisibility decides complete-matrix membership in asymmetric two-shear groups for `u+v>4`, with a documented sign correction to earlier work |
