...
`D_b;D_c;D_c` backward ray. It proves a first-letter exclusion for the `MM-S86` slope chamber
...
Consequently every physical predecessor entering the `MM-S86` slope interval is `c`-leading.
...
chamber: a valid `MM-S86` carrier can have next slope greater than one. Nor does the slope
...
target is arithmetic rather than Archimedean. Pull the `MM-S86` divisibility channel backward
