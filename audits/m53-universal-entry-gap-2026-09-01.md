...
does not by itself satisfy the two-sided intercept hypotheses of the `MM-S105` no-reentry
automaton.
...
`MM-S104` and the target-code divisor from `MM-S92` must either force a quotient-scale lower bound
for `Ξ`, allowing the `MM-S105` automaton to close, or expose a physical family with arbitrarily
